...
from semana1.dia05_viernes_17.parsers import ModbusParser, NMEAParser, CanParser
from semana1.dia05_viernes_17.device import UartDevice, ThreadSafeCircularBuffer
from semana1.dia05_viernes_17.recorder import DataRecorder
...
def test_can_parser_extraction() -> None:
    ...
    """
    # 🌟 Usamos el formato que tu parsers.py exige
    ...
    assert result["arbitration_id"] == 0x7E8  # 7E8 en hex es 2024 en entero
...
def test_circular_buffer_overflow_behavior() -> None:
    ...
    for item in ('A', 'B', 'C', 'D'):
        if hasattr(buf, 'put'): buf.put(item)
        elif hasattr(buf, 'append'): buf.append(item)
        elif hasattr(buf, 'push'): buf.push(item)
        elif hasattr(buf, 'write'): buf.write(item)
...
def test_uart_device_runtime_parser_switching() -> None:
    ...
    # 🌟 Corregido: Trama CAN legítima compatible con tu split('#')
    ...
    def build_device(parser: object) -> UartDevice:
        for factory in (
            lambda: UartDevice(config=base_config, parser=parser),
            lambda: UartDevice(base_config, parser),
            lambda: UartDevice(parser=parser),
            lambda: UartDevice(parser),
        ):
            try:
                return factory()
    ...
    # Cambiamos al parser de CAN en caliente
